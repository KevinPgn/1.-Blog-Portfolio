...
const Links = ({userRole}: {userRole: string}) => {
  ...
  return <div className="flex items-center gap-10 max-lg:hidden">
    ...
    {userRole === "admin" && <Link href="/admin" className={cn("hover:text-gray-500 text-sm font-semibold duration-75 uppercase text-md tracking-wider", pathname === "/admin" && "text-blue-500 underline hover:text-blue-500")}>Create Post</Link>}
